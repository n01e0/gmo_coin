...
fn status_returns_known_exchange_status() {
    ...
    assert!(
        matches!(resp.data.status.as_str(), "OPEN" | "PREOPEN" | "MAINTENANCE"),
...
fn trades_returns_non_empty_list() {
    let resp = call_with_retry("public::api::trades", || api::trades(Symbol::BTC, Some(1), Some(5)));
...
fn klines_returns_candles_for_sample_date() {
    ...
    assert!(!resp.data[0].open_time.is_empty(), "open_time must not be empty");
}
